...
// 자(0)→0:30, 축(1)→2:30, ... 해(11)→22:30 — 각 시지의 중앙 시간
...
// 한국 표준시(시계시간) → 진태양시 변환 (분 단위 오프셋)
function trueSolarOffsetMin(date: Date, lon: number): number {
  return -(KST_STD_LON - lon) * 4 + equationOfTimeMin(date);
}

/**
 * 생년월일(+시간)로 사주 명식을 계산한다.
 * lunar-javascript 기반 — 입춘·각월 절입 시간, 23시 자시 경계까지 반영된 정식 만세력.
...
export function computeSaju(birth: string, hourIdx: number | null): SajuResult {
  ...
  const tst = new Date(
    civil.getTime() + trueSolarOffsetMin(civil, DEFAULT_LON) * 60000,
  );
