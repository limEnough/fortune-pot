...
function NewMapForm({
  ...
  onCreate: (i: JoinInput) => Promise<unknown>;
}) {
  return (
    <JoinForm
      title={<>🌟 내 귀인지도 그리기</>}
      desc={
        <>
          생일만 넣으면 바로 나와요. 지도를 만들고 링크를 공유하면, 친구가 자기
          생일을 넣고 내 지도에 별로 올라와요.
        </>
      }
...
export default function MapPage() {
  const { ready, hasMap, id, map, loading, error, create, reload, remove } =
    useMyMap();
  const { saju } = useSaju();
  ...
  if (!hasMap) {
     ...
     * 시간·성별을 다 가지고 있으므로 버튼 하나면 된다. 지도를 만드는 건 서버에
          ...
          <div className="scroll">
            <div className="join-card">
              ...
              <p className="join-desc">
                <b>{saju.name}</b>님({saju.birth}) 정보로 지도를 만들어요.
                지도를 만들고 링크를 공유하면, 친구가 자기 생일을 넣고 내 지도에
                별로 올라와요.
              </p>
            ...
            <p className="map-note">
              지도에는 이름과 유형만 보여요. 생년월일은 궁합을 계산할 때만
              쓰이고 다른 사람에게 보이지 않아요.
            </p>
          </div>
        </section>
      ...
      <div className="scroll">
        ...
        {error && !map && (
          <div className="map-empty">
            <p>{error}</p>
            <button className="btn ghost focusable" onClick={reload}>
              다시 시도
            </button>
          </div>
        )}
        ...
        {map && (
          <div className="map-screen">
            <div className="map-card">
              ...
              <p className="join-desc">
                이 링크를 받은 사람이 생일을 넣으면 <b>{map.owner.name}</b>님
                지도에 별로 올라와요. 이름을 올린 사람은{" "}
                <b>이 지도를 볼 수 있어요</b>
            ...
            <MapBoard
              ...
              headExtra={
                <button
                  className="link-btn refresh focusable"
                  onClick={reload}
                  disabled={loading}
                >
                  {loading ? "새로고침 중…" : "새로고침"}
