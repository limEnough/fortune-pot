...
- **지도 정보를 사주로 이어쓰기.** 지도를 만들거나 이름을 올리면 그 값이 사주
  ...
  시각은 그 화면으로 갈 때 입력 폼에서 받습니다 — 폼은 나머지 칸이 채워진 채로
  뜨고 빈 칸에 *확인 필요* 표시가 붙습니다(`isComplete()` 가 판정).
...
- **최근 정보 이어쓰기.** 저장된 사주가 있으면 메인 정중앙 팝업으로 먼저 묻고,
  ...
  건너뜀). 메인에는 *◯◯님 정보로 이용중이에요* 안내와 초기화 버튼이 뜹니다.
...
- **LNB 에 세 화면이 늘 다 보입니다.** 예전엔 사주가 없으면 운세·사주 항목을
  ...
  있는 화면은 *지금 화면* 표시로 남깁니다. 귀인지도 화면들도 같은 헤더(햄버거)를
  씁니다.
...
- **입력 화면이 둘로 갈라졌습니다.** `/infoinput?next=` 는 저장 후 고른 화면으로
  ...
  `/onboarding` 하나가 둘을 겸해서, 정보를 고치러 들어와도 *"최근 조회한 ◯◯님으로
  오늘의 운세를 확인해보시겠어요?"* 가 맨 위에 떴습니다. `/onboarding` 은
