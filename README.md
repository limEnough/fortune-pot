...
src/
  ...
  components/
    ...
    map/                                   StarMap(별지도) · JoinForm · JoinPanel
                                           ChemiList(케미 나래비) · ShareBar
  ...
  store/                  Zustand (게스트 사주 / 지도 id·열쇠 / UI 드로어 / 릴리즈 확인 상태)
...
```
나  ─ 생일 입력 → 지도 생성(id + 열쇠) → 링크 공유
                                            │
친구 ─ 링크 열기 → 자기 생일 입력 → 내가 그 사람에게 어떤 사람인지 즉시 확인
                                            └→ 내 지도에 별로 추가 (다음 방문에 보임)
```
...
- 링크(`/map/{id}`)만으로 보이는 건 **주인 이름과 인원수**까지입니다. 지도를 열려면
  주인 브라우저(localStorage `fortunepot-map`)에 있는 `key` 가 필요합니다.
  비회원 앱이라 이 값을 잃으면 되찾을 방법이 없습니다.
- 보관 기간은 마지막 쓰기로부터 200일, 한 지도에 60명까지입니다.

### 입력

여섯 자리(`930821`)와 양/음력 선택만 받습니다. 사주 입력 폼과 달리 시각·성별을 받지
않는 건, 궁합 기준이 일간이라 시각 없이도 나오고 **남의 링크에서 채울 칸은 적을수록**
좋기 때문입니다. 음력은 `lunarToSolar` 로 양력에 옮겨 계산합니다(윤달 미지원).
